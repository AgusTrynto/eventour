...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('event_organizers', function (Blueprint $table) {
            $table->string('bank_channel_code')->nullable()->after('bank_name');
        });
        ...
        foreach ($bankMap as $bankName => $channelCode) {
            DB::table('event_organizers')
                ->where('bank_name', $bankName)
                ->whereNull('bank_channel_code')
                ->update(['bank_channel_code' => $channelCode]);
        }

        Schema::table('payouts', function (Blueprint $table) {
            $table->string('xendit_payout_id')->nullable()->index()->after('transfer_proof');
    ...
    public function down(): void
    {
        ...
        Schema::table('event_organizers', function (Blueprint $table) {
            $table->dropColumn('bank_channel_code');
        });
    }
};
